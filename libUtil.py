...
def createNormalPlane(objArray=None, name='normalPlane'):
    ...
    pm.delete(
        pm.aimConstraint(objArray[-1], normalPlane, aimVector=[1, 0, 0], upVector=[0, 1, 0], mo=0, worldUpVector=smpN))
...
def createPlaceHolder(objArray=None, cnx=0, grpLoc='locatorGrp', setObj='objSet'):
    ...
    for x in objArray:
        ...
        if pm.objExists(x.nodeName() + '_loc'):
            N = pm.PyNode(x.nodeName() + '_loc')
            print 'deleteing %s' % N
            pm.delete(N)

        l = pm.spaceLocator(n=x.nodeName() + '_loc')
        cons = []

        if x.attr('tx').isLocked() == 0 and x.attr('ty').isLocked() == 0 and x.attr('tz').isLocked() == 0 and x.attr(
                'rx').isLocked() == 0 and x.attr('ry').isLocked() == 0 and x.attr('rz').isLocked() == 0:
        ...
        if cnx == 0:
            ...
            if x.attr('tx').isLocked() == 0 and x.attr('ty').isLocked() == 0 and x.attr(
                    'tz').isLocked() == 0 and x.attr('rx').isLocked() == 0 and x.attr('ry').isLocked() == 0 and x.attr(
                    'rz').isLocked() == 0:
...
def deleteATTR(sel=None):
    ...
    for obj in sel:
        ...
        listAttrs = ['visibility', 'translateX', 'translateY', 'translateZ', 'rotateX', 'rotateY', 'rotateZ', 'scaleX',
                     'scaleY', 'scaleZ']
        for A in attrs:
...
def deleteChildrenConstraints(objArray=None):
    ...
    for obj in cleanObjs:
        for ch in pm.listRelatives(obj, c=1):
            if ch.type() in ['parentConstraint', 'pointConstraint', 'orientConstraint', 'aimConstraint',
                             'scaleConstraint', 'geometryConstraint']:
                try:
                    pm.delete(ch)
...
def dynamicParent(sel=None):
    ...
    pm.select(objAUTO)
    parentNames = []
    if namesAUTO[3] == "arm" or namesAUTO[0] == "LFrt" or namesAUTO[0] == "RFrt" or namesAUTO[
        1] == "armLo" and num == 6:
        parentNames = ["head", "chest", "main", "hip", "direction"]
    if namesAUTO[3] == "arm" and num == 3:
        parentNames = ["main", "arm"]
    if namesAUTO[3] == "leg" or namesAUTO[3] == "legFrnt" or namesAUTO[3] == "legMid" or namesAUTO[
        3] == "legBack" and num == 3:
...
def dupAndRenameDetail(detail, obj=None):
    ...
    for i in xrange(len(selNewJnt)):
        ...
        if selNewJnt[i].nodeType() == 'parentConstraint' or selNewJnt[i].nodeType() == 'pointConstraint' or selNewJnt[
            i].nodeType() == 'orientConstraint' or selNewJnt[i].nodeType() == 'aimConstraint':
...
def locOnCrvEpsilon(smpName, smpCrv, smpNumberLoc=10.0, smpEpsilon=0.0001, ch=0,
    ...
    for i in range(1, smpNumberLoc + 1):
        ...
        if ch == 0:
            pm.disconnectAttr(pm.connectionInfo(locPosTemp + ".translate", sourceFromDestination=True),
                              locPosTemp + ".translate")
...
def matchPivot(sel=None, axis='xyz'):
    ...
    for tar in targets:
        ...
        if 'x' in axis: scale, rotate = [srcScale[0], tarScale[1], tarScale[2]], [srcRotate[0], tarRotate[1],
                                                                                  tarRotate[2]]
        if 'y' in axis: scale, rotate = [tarScale[0], srcScale[1], tarScale[2]], [tarRotate[0], srcRotate[1],
                                                                                  tarRotate[2]]
        if 'z' in axis: scale, rotate = [tarScale[0], tarScale[1], srcScale[2]], [tarRotate[0], tarRotate[1],
                                                                                  srcRotate[2]]
...
def movePivotMinY(sel=None, moveTo='minY'):
    ...
    for obj in sel:
        ...
        if moveTo == 'minX':
            x, y, z = bb[0], piv.y, piv.z
        elif moveTo == 'maxX':
            x, y, z = bb[3], piv.y, piv.z
        elif moveTo == 'minY':
            x, y, z = piv.x, bb[1], piv.z
        elif moveTo == 'maxY':
            x, y, z = piv.x, bb[4], piv.z
        elif moveTo == 'minZ':
            x, y, z = piv.x, piv.y, bb[2]
        elif moveTo == 'maxZ':
            x, y, z = piv.x, piv.y, bb[5]
...
def makeAimLine(sel=None):
    ...
    cleanUpAttr(sel=[clus01[1], clus02[1], crv, Grp],
                listAttr=['tx', 'ty', 'tz', 'rx', 'ry', 'rz', 'sx', 'sy', 'sz', 'v', 'radi'], l=0, k=1, cb=1)
    # hide cluster
...
def quickMirror(objArray=None, upVector=[0, 0, 1], axis='X'):
    ...
    for obj in objArray:
        ...
        if pm.objExists(mirrorObj) == 0:
            ...
            aimCon = pm.aimConstraint(locAim, mloc, aimVector=[1, 0, 0], upVector=upVector, worldUpObject=locUp,
                                      worldUpType='object', mo=0)
        ...
        for at in myAttrs:
            if x.attr(at).isCompound():
                ...
                if numC != 0 and indices is not None:
                    for c in x.attr(at).children():
                        ...
                        for i in indices:
                            ...
                            scriptListSetAttr.append(
                                setAttrCmd[0] + n + setAttrCmd[1] + myChildAt + setAttrCmd[2] + str(v) + setAttrCmd[3])
                ...
                if pm.objExists(x + '.' + at):
                    ...
                    v = x.attr(at).get()
                    scriptListSetAttr.append(
                        setAttrCmd[0] + n + setAttrCmd[1] + at + setAttrCmd[2] + str(v) + setAttrCmd[3])
...
def setSDK(attrDrv, attrDrvn, valDrv, valDrvn, typeKey='linear', infinity=1, sel=None):
    ...
    for i in range(0, len(attrDrvn)):
        for a in range(0, len(valDrv[i])):
            pm.setDrivenKeyframe(driven, cd=(driver + '.' + attrDrv), at=attrDrvn[i], v=valDrvn[i][a], dv=valDrv[i][a],
                                 itt=typeKey, ott=typeKey)
...
def snapWithVertices(driver=None, driven=None, vertices=None):
    ...
    for geo in drivenS, driverS:
        ...
        for cv in cvs:
            ...
            i = i + 1
        pm.aimConstraint('%sLoc2' % name, '%sLoc1' % name, weight=1, upVector=(0, 1, 0), mo=1,
                         worldUpObject='%sLoc3' % name, worldUpType="object", aimVector=(1, 0, 0))
        dvr = 1
...
def transferAttr(driven, ctrl, attrArray=None, header=None):
    ...
    for at in attrArray:
        ...
        if header is not None:
            gmatName = header + pm.util.capitalize(at)
        else:
